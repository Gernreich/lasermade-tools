...
**The cause is fixed too, for the case that bites.** `md2html.py` now joins a
wrapped image back onto one line before matching it, so an alt text may run to as
many lines as it needs. A run that never closes is left exactly as it was, so
prose that merely opens with those two characters is untouched. Re-rendering all
twenty published pages with the change produces twenty byte-identical files: the
...
path resolves* read the raw source, so this README reporting on the check above
was itself reported as linking to a file called "path". It reads the prose now,
with fences and inline code spans blanked. *every file the document names exists*
still reads the raw source deliberately: "generated by `bell.py`" is a real
reference to a real file, and that looser claim is what it is there to catch.
...
Fourteen ribbon bore runs over the six open shapes, two over the torus and one
on `--port-at`, `regress.py` over 24 block designs, `doc-audit` over
