...
# A LINK INSIDE BACKTICKS IS QUOTED SYNTAX, NOT A LINK. This read the raw source,
# so a writeup that explains image markdown -- this repository's own README, as soon
# as it documented the check above -- was reported as linking to a file called
# "path". Fences are already blanked for every other prose check; blank them here
# too, and inline code spans with them. The filename check further up still reads
# the RAW source on purpose: "generated by bell.py" in backticks is a real
# reference to a real file, and that is the looser claim it is meant to catch.
link_src = re.sub(r"`[^`\n]*`", " ", prose_src)
unresolved = sorted({u for u in link_targets(link_src)
